...
class HistoryManagerTests(unittest.TestCase):
    def test_retention_trims_oldest_sessions_on_load(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            repository = SessionDirectoryRepository(Path(temp_dir) / "sessions")
            ...
            retained_ids = [session.entity_id for session in manager.list_sessions()]
            self.assertEqual(retained_ids, [session.entity_id for session in sessions[-2:]])
    ...
    def test_enabling_policy_cleans_existing_history(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            retained_ids = [session.entity_id for session in manager.list_sessions()]
            self.assertEqual(retained_ids, [session.entity_id for session in sessions[-3:]])
