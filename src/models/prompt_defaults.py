...
LEGACY_TEXT_REPLY_PROMPT = (
    ...
    "and easy to speak aloud. Be concise by default, but include enough detail "
    "to genuinely help. Prefer natural sentences over lists. Do not use "
    "markdown, code fences, or visual formatting unless the user explicitly "
    "asks for them."
)
...
LEGACY_VOICE_REPLY_PROMPT = (
    ...
    "natural, and only give longer answers when the user is clearly asking for "
    "more. Small conversational turns should usually be one short sentence, or "
    "two short sentences at most. Avoid rambling, avoid repeating the same "
    ...
    "writing. Do not use markdown, code fences, bullets, or visual formatting. "
    "Do not explain your process. Do not rewrite, critique, or correct another "
    "assistant message. Do not change speaker identity or perspective. Do not "
    ...
    "tags may stay sparse. Use them freely when useful, but do not overdo them "
    "or make the result chaotic. Use only square-bracket Eleven-style tags "
    "such as [excited], [laughs], [sighs], [whispers], [curious], "
    "[mischievously], [swallows], [strong French accent], or [applause]. Never "
    "use angle-bracket tags like <laugh>, never use emoji, never use SSML, and "
...
LEGACY_TTS_PREPARATION_PROMPT = (
    ...
    "Eleven v3 speech generation. Your primary goal is to prepare final spoken "
    "text that sounds expressive, engaging, and natural while strictly "
    ...
    "include at least one suitable Eleven-style tag when it improves delivery. "
    "For neutral factual replies, tags may stay sparse. Use them strategically "
    "and freely when useful, but do not make the output chaotic or theatrical. "
    "Use only square-bracket Eleven-style tags such as [excited], [laughs], "
    "[sighs], [whispers], [curious], [mischievously], [swallows], [strong "
    "French accent], or [applause]. Never use angle-bracket tags like <laugh>, "
    "never use emoji, never use SSML, and never invent non-auditory stage "
...
def normalize_prompt_value(field_name: str, value: object) -> str:
    ...
    if not stripped_text:
        return default
    if stripped_text == default or stripped_text in LEGACY_PROMPT_DEFAULTS[field_name]:
        return default
    return text
