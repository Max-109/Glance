...
ELEVEN_V3_VOICE_LABELS = {
    AUTO_TTS_VOICE_ID: "Auto",
    **{
        voice.id: f"{voice.name} - {voice.title}"
...
class AppSettings:
    ...
    open_glance_keybind: str = "CMD+SHIFT+G"
    llm_base_url: str = ""
    llm_api_key: str = ""
    ...
    tool_change_memory_policy: str = DEFAULT_TOOL_POLICY
    ...
    def validate(self) -> None:
        ...
        self.open_glance_keybind = normalize_keybind(
            self.open_glance_keybind
        )
        ...
        if self.transcription_reasoning not in {
            ...
            raise ValidationError(
                "transcription_reasoning must be minimal, low, medium, "
                "or high."
            )
    ...
    def from_mapping(
        ...
        settings = cls(
            ...
            open_glance_keybind=normalize_keybind(
                data.get(
                    "open_glance_keybind",
                    data.get("open_menu_keybind", cls.open_glance_keybind),
                )
            ),
            ...
            transcription_reasoning=normalize_llm_reasoning(
                data.get("transcription_reasoning",
                         cls.transcription_reasoning)
            ),
            multimodal_live_enabled=coerce_bool(
                data.get("multimodal_live_enabled",
                         cls.multimodal_live_enabled)
            ),
            ...
            history_retention_enabled=coerce_bool(
                data.get("history_retention_enabled",
                         cls.history_retention_enabled)
            ),
            ...
            tool_read_memory_policy=normalize_tool_policy(
                data.get("tool_read_memory_policy",
                         cls.tool_read_memory_policy)
            ),
            tool_change_memory_policy=normalize_tool_policy(
                data.get(
                    "tool_change_memory_policy",
                    cls.tool_change_memory_policy,
                )
            ),
            ...
            screen_change_threshold=float(
                data.get("screen_change_threshold",
                         cls.screen_change_threshold)
            ),
            ...
            audio_endpoint_patience=normalize_endpoint_patience(
                data.get("audio_endpoint_patience",
                         cls.audio_endpoint_patience)
            ),
            ...
            audio_max_record_seconds=float(
                data.get("audio_max_record_seconds",
                         cls.audio_max_record_seconds)
            ),
